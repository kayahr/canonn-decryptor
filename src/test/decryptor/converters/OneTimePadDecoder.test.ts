...
describe("OneTimePadDecoder", () => {
    describe("constructor", () => {
        it("initializes to empty pad if none given", () => {
            expect(new OneTimePadDecoder().pad).toBe("");
        });
        it("initializes to given pad", () => {
            expect(new OneTimePadDecoder({ pad: "Canonn" }).pad).toBe("Canonn");
        });
        ...
        it("creates correct pad codes when pad is given", () => {
            expect(new OneTimePadDecoder({ pad: "OneTimePad" }).getPadCodes())
                .toEqual([ 14, 13, 4, 19, 8, 12, 4, 15, 0, 3 ]);
            expect(new OneTimePadDecoder({ pad: "The quick brown fox jumps over the lazy dog" }).getPadCodes())
    ...
    describe("setPad", () => {
        it("updates the pad", () => {
            const decoder = new OneTimePadDecoder({ pad: "Not this one" });
            decoder.pad = "Canonn";
            expect(decoder.pad).toBe("Canonn");
        });
    ...
    describe("convert", () => {
        it("converts empty string to empty string", () => {
            expect(new OneTimePadDecoder({ pad: "Imperial Clipper" }).convert("")).toBe("");
        });
        it("keeps white-space only string", () => {
            expect(new OneTimePadDecoder({ pad: "Imperial Cutter" }).convert(" \n\r\t")).toBe(" \n\r\t");
        });
        it("keeps unencoded characters", () => {
            expect(new OneTimePadDecoder({ pad: "Sidewinder" }).convert("1ö_<")).toBe("1ö_<");
        });
        it("decodes normal lower-case characters", () => {
            expect(new OneTimePadDecoder({ pad: "Eagle Mk3" }).convert("joumed")).toBe("foobar");
        });
        it("decodes normal upper-case characters", () => {
            expect(new OneTimePadDecoder({ pad: "Asp Explorer" }).convert("FGDFXG")).toBe("FOOBAR");
        });
        it("decodes normal mixed-case characters", () => {
            expect(new OneTimePadDecoder({ pad: "Anaconda" }).convert("FboDoe")).toBe("FooBar");
        });
        it("decodes only normal characters in mixed string", () => {
            expect(new OneTimePadDecoder({ pad: "Cobra Mk3" }).convert("#12HcpSad!")).toBe("#12FooBar!");
        });
    });

    describe("toJSON", () => {
        it("serializes the converter", () => {
            expect(new OneTimePadDecoder({ pad: "Secret" }).toJSON()).toEqual({
        ...
        it("does not serialize default option values", () => {
            expect(new OneTimePadDecoder({ pad: "" }).toJSON()).toEqual({
                type: "one-time-pad-decoder"
            });
        });
    });
